...
Evaluator `0.6.0` preserves the deterministic media, subtitle, brand-text, and screen-policy checks and uses two complementary Gemini 3.6 Flash protocols. The pairwise whole-video judge runs blind passes with reversed A/B order. Before a generated hook is selectable, the temporal judge samples its declared scene at 10 FPS and inspects timestamped adjacent-frame strips for a closed set of continuity failures. A reported high-severity event blocks automatic selection until its evidence frames are reviewed; confirmed defects veto and confirmed false positives may be cleared. Both judges store sanitized structured evidence and metrics are calculated offline. `visual_judge_win_rate` is a diagnostic model-preference signal. Explicit product-owner acceptance of the retained artifact controls reviewed product decisions after temporal consistency, `timeline_alignment_f1`, technical validity, screen policy, and brand evidence pass. Timestamped ASR and rendered-audio pronunciation remain pending.
