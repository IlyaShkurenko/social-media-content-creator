...
### VOICE-1 — Instructable hook narration

#### VOICE-1.1 — Delivery direction describes character, not audio mechanics

Each `AdvertisingConcept` MUST include `hook_voice_delivery`, generated by the same planning call that produces `hook_voiceover` and `target_emotion`. It MUST describe the speaker's emotional state and situation and MUST NOT issue literal audio-engineering directions (volume, syllable stress, pacing percentages, fade behaviour); an instructable narration model performs worse when told how to manipulate its own delivery mechanically. `compile_concept_storyboards` MUST copy it onto the compiled hook scene as `StoryboardScene.voice_instructions`; other scenes MUST leave it unset.

#### VOICE-1.2 — Paid narration providers share the campaign budget scope

A narration voice that resolves to a paid provider MUST reserve and charge the shared `mixed-media-iteration-001` budget scope before and after every synthesis call, fail-closed, with no automatic resubmission of an operation ID that already has a ledger entry. The existing free default voice MUST NOT touch the budget ledger.
